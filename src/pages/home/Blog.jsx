import React from "react";
import Cardsquare from "../../components/cards/Cardsquare";
import content from "../../content";

const Blog = () => {
  return (
    <>
      <img
        className="mx-auto w-10 mb-2 skew-x-12 -skew-y-12"
        src="data:image/png;base64,"
      ></img>
      <h1 className="text-center text-blue-dark font-semibold text-2xl mb-3 ">
        Our Latest Blog
      </h1>
      <h1 className="text-center text-blue-dark text-3xl md:text-5xl font-semibold">
        Our Blog and New Releases
      </h1>
      <div className="grid grid-cols-1sm:grid-cols-2 lg:grid-cols-3 px-4 md:px-2 lg:px-0 mx-auto gap-6">
          {content.Blog.slice(0, 3).map((item) => (
        <div className="flex flex-1 object-contain my-10 md:my-20">
            <Cardsquare
              key={item.id}
              id={item.id}
              img={item.img}
              text={item.heading}
              cardClass={
                "w-full max-w-[80%] h-full px-0 pb-4 pt-0"
              }
              imgClass={
                "py-0 -mt-5 mb-4 max-h-[50%]  rounded-t-md object-cover w-full h-full"
              }
              paraClass={"px-4 w-full font-semibold text-xl overflow-hidden text-left min-h-[30%] "}
              innerTextClass={'line-clamp-3'}
              buttonClass={'pt-0 px-0 py-0 pr-0'}
              button="Read more"
              function=""
            />
        </div>
          ))}
      </div>
    </>
  );
};

export default Blog;
